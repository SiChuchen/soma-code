...
const COORDINATOR_DIRECT_TOOL_NAMES = [
  AGENT_TOOL_NAME,
  SEND_MESSAGE_TOOL_NAME,
  TASK_STOP_TOOL_NAME,
  SYNTHETIC_OUTPUT_TOOL_NAME,
...
export function getCoordinatorDirectToolNames(): string[] {
  return COORDINATOR_DIRECT_TOOL_NAMES.filter(name =>
    COORDINATOR_MODE_ALLOWED_TOOLS.has(name),
  )
}

function describeCoordinatorDirectTools(): string {
  return getCoordinatorDirectToolNames().join(', ')
}

function describeCoordinatorWorkerTools(): string {
...
export function getCoordinatorUserContext(
  ...
  const workerTools = isEnvTruthy(process.env.CLAUDE_CODE_SIMPLE)
    ? [BASH_TOOL_NAME, FILE_READ_TOOL_NAME, FILE_EDIT_TOOL_NAME]
        .sort()
        .join(', ')
    : describeCoordinatorWorkerTools()

  let content =
    `Coordinator direct tools: ${describeCoordinatorDirectTools()}\n\n` +
...
export function getCoordinatorSystemPrompt(): string {
  ...
  const directTools = describeCoordinatorDirectTools()

  return `You are somacode, an AI assistant that orchestrates software engineering tasks across multiple workers.
...
- **${TOOL_SEARCH_TOOL_NAME}** - Inspect the currently available tool surface before delegating
- **${LIST_MCP_RESOURCES_TOOL_NAME}** / **${READ_MCP_RESOURCE_TOOL_NAME}** - Inspect MCP resources directly when you need context for routing or summarization
- **subscribe_pr_activity / unsubscribe_pr_activity** (if available) - Subscribe to GitHub PR events (review comments, CI results). Events arrive as user messages. Merge conflict transitions do NOT arrive — GitHub doesn't webhook \`mergeable_state\` changes, so poll \`gh pr view N --json mergeable\` if tracking conflict status. Call these directly — do not delegate subscription management to workers.

Your direct tool surface is intentionally limited to: ${directTools}

Do not try to do repository reads, edits, writes, notebook changes, or bash execution from the coordinator thread. If real code or filesystem work is required, delegate it to a worker.
...
When calling ${AGENT_TOOL_NAME}, choose among the coordinator worker roles exposed in the tool schema. Default to \`general-purpose\` for implementation, use \`Explore\` for read-only investigation, and use \`Plan\` when you want a worker to structure an approach before code changes.
