...
import { PROJECT_CONFIG_DIR_NAME } from 'src/utils/configPaths.js';
...
import { getAppXDGStateDirCandidates } from '../utils/xdg.js';
type Props = {
...
function DistTagsDisplay(t0: DistTagsDisplayProps): React.ReactNode {
  const $ = _c(8);
  const {
    promise
  } = t0;
...
export function Doctor(t0) {
  ...
  if ($[6] !== agentDefinitions || $[7] !== toolPermissionContext || $[8] !== tools) {
    t5 = () => {
      ...
      (async () => {
        ...
        const projectAgentsDir = join(getOriginalCwd(), PROJECT_CONFIG_DIR_NAME, "agents");
        const lockDirCandidates = getAppXDGStateDirCandidates().map(dir => join(dir, "locks"));
        const {
          activeAgents,
          allAgents,
          failedFiles
        } = agentDefinitions;
        const [userDirExists, projectDirExists, lockDirExists] = await Promise.all([pathExists(userAgentsDir), pathExists(projectAgentsDir), Promise.all(lockDirCandidates.map(candidate => pathExists(candidate)))]);
        const agentInfoData = {
        ...
        if (isPidBasedLockingEnabled()) {
          const locksDir = lockDirCandidates[lockDirExists.findIndex(Boolean)] ?? lockDirCandidates[0] ?? "";
  ...
  if ($[11] !== onDone) {
    t7 = () => {
      onDone("somacode diagnostics dismissed", {
        display: "system"
      });
    };
    $[11] = onDone;
