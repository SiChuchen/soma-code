...
type BunListenSocket = {
  data: ConnState & { writeBuf: Uint8Array[] }
...
function startBunRelay(
  ...
  const server = Bun.listen({
    ...
    port: 0,
    socket: {
      open(sock: BunListenSocket) {
        sock.data = { ...newConnState(), writeBuf: [] }
      },
      data(sock: BunListenSocket, data: Uint8Array) {
        const st = sock.data
      ...
      close(sock: BunListenSocket) {
        cleanupConn(sock.data)
      },
      error(sock: BunListenSocket, err: Error) {
...
function openTunnel(
  ...
  ws.onopen = () => {
    ...
    ws.send(toWebSocketPayload(encodeChunk(Buffer.from(head, 'utf8'))))
...
function sendKeepalive(ws: WebSocketLike): void {
  if (ws.readyState === WebSocket.OPEN) {
    ws.send(toWebSocketPayload(encodeChunk(new Uint8Array(0))))
  }
}
...
function forwardToWs(ws: WebSocketLike, data: Buffer): void {
  ...
  for (let off = 0; off < data.length; off += MAX_CHUNK_BYTES) {
    ...
    ws.send(toWebSocketPayload(encodeChunk(slice)))
  }
}

function toWebSocketPayload(data: Uint8Array): ArrayBuffer {
