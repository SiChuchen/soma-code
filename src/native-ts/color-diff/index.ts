...
function hljs(): HLJSApi {
  ...
  const mod = require('highlight.js') as unknown
  // highlight.js uses CJS. Under some runtimes the interop wraps the API in
  // .default; under others the module itself is the API.
  const candidate =
    typeof mod === 'object' &&
    mod !== null &&
    'default' in mod &&
    ...
    typeof mod.default === 'object'
      ? mod.default
      : mod
  cachedHljs = candidate as HLJSApi
  return cachedHljs!
}
...
// highlight.js has exposed both emitter and _emitter shapes across versions /
// interop modes. Normalize at the local adapter boundary, then validate the
// token-tree shape once so we fail loudly instead of silently losing coloring.
...
function getHighlightEmitter(result: HLJSHighlightResult): unknown {
  return result._emitter ?? result.emitter
}
...
function highlightLine(
  ...
  if (!hasRootNode(emitter)) {
    if (!loggedEmitterShapeError) {
      ...
      logError(
        new Error(
          `color-diff: hljs emitter shape mismatch (keys: ${typeof emitter === 'object' && emitter !== null ? Object.keys(emitter).join(',') : typeof emitter}). Syntax highlighting disabled.`,
        ),
      )
    }
    return [[defaultStyle(theme), code]]
