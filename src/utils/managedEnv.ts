...
import { applyAnthropicCompatProfileDefaults } from './anthropicCompatConfig.js'
import { clearCACertsCache } from './caCerts.js'
import { getGlobalConfig } from './config.js'
import { isEnvTruthy } from './envUtils.js'
import { applyOpenAICompatProfileDefaults } from './openaiCompatConfig.js'
import {
 ...
 * env vars that the remote's ~/.soma settings.env MUST NOT clobber (see
 ...
 * user's ~/.soma/settings.json can't redirect requests away from the
...
let initialApiConfigEnvSnapshot: Record<string, string | undefined> | undefined

function captureInitialApiConfigEnvSnapshot(): void {
  if (initialApiConfigEnvSnapshot !== undefined) {
    return
  }

  initialApiConfigEnvSnapshot = {}
  for (const key of API_CONFIG_RUNTIME_ENV_KEYS) {
    initialApiConfigEnvSnapshot[key] = process.env[key]
  }
}

function resetApiConfigEnvironment(): void {
  captureInitialApiConfigEnvSnapshot()

  for (const key of API_CONFIG_RUNTIME_ENV_KEYS) {
    const value = initialApiConfigEnvSnapshot?.[key]
    if (value === undefined) {
...
function getSettingsEnvForApplication(
  ...
  const env = settings.api ? stripApiConfigEnv(settings.env) : { ...(settings.env ?? {}) }

  return settings.api
    ? {
        ...env,
        ...materializeApiConfigEnv(settings.api),
      }
    : env
}
 ...
 *
 * - userSettings (~/.soma/settings.json): controlled by the user, not project-specific
...
export function applySafeConfigEnvironmentVariables(): void {
  ...
  // Global config (~/.soma.json) is user-controlled. In CCD mode,
  ...
  // doesn't get clobbered by ~/.soma/settings.json env (gh#217). policy/flag
  ...
  for (const source of TRUSTED_SETTING_SOURCES) {
    ...
    Object.assign(
      process.env,
      filterSettingsEnv(getSettingsEnvForApplication(getSettingsForSource(source))),
    )
  }
  applyAnthropicCompatProfileDefaults(process.env)
  applyOpenAICompatProfileDefaults(process.env)
  ...
  Object.assign(
    process.env,
    filterSettingsEnv(
      getSettingsEnvForApplication(getSettingsForSource('policySettings')),
    ),
  )
  applyAnthropicCompatProfileDefaults(process.env)
  applyOpenAICompatProfileDefaults(process.env)
  ...
  const settingsEnv = filterSettingsEnv(
    getSettingsEnvForApplication(getSettings_DEPRECATED()),
  )
...
export function applyConfigEnvironmentVariables(): void {
  ...
  Object.assign(
    process.env,
    filterSettingsEnv(getSettingsEnvForApplication(getSettings_DEPRECATED())),
  )
  applyAnthropicCompatProfileDefaults(process.env)
  applyOpenAICompatProfileDefaults(process.env)
