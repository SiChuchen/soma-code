...
import { getAppTempDir } from '../permissions/filesystem.js'
...
/**
 * Resolve somacode-specific path patterns for sandbox-runtime.
 *
 * somacode uses special path prefixes in permission rules:
...
/**
 * Convert somacode settings format to SandboxRuntimeConfig format
...
export function convertToSandboxRuntimeConfig(
  ...
  const allowWrite: string[] = ['.', getAppTempDir()]
  const denyWrite: string[] = []
  const denyRead: string[] = []
  const allowRead: string[] = []

  // Always deny writes to settings.json files to prevent sandbox escape
  // This blocks settings in the original working directory (where somacode started)
  ...
  const networkConfig: NetworkRestrictionConfig = {
  ...
  const readConfig: FsReadRestrictionConfig = {
    denyOnly: denyRead,
    ...(allowRead.length > 0 && { allowWithinDeny: allowRead }),
  }
  const writeConfig: FsWriteRestrictionConfig = {
    allowOnly: allowWrite,
    denyWithinAllow: denyWrite,
  }
