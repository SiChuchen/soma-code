/**
 * Marketplace manager for somacode plugins
 *
 ...
 *
 * Reads the configuration file at ~/.soma/plugins/known_marketplaces.json
...
export async function loadKnownMarketplacesConfigSafe(): Promise<KnownMarketplacesConfig> {
  try {
    ...
    // corrupted user config isn't a somacode bug, shouldn't hit the error file.
    return {}
  }
}
 ...
 *
 * Writes the configuration to ~/.soma/plugins/known_marketplaces.json,
 ...
 * The configuration is saved to ~/.soma/plugins/known_marketplaces.json.
 *
  ...
  if (seedDir) {
    throw new Error(
      `Marketplace '${name}' is registered from the read-only seed directory ` +
        ...
        `To stop using its plugins: soma plugin disable <plugin>@${name}`,
    )
  }
    ...
    if (
      ...
      throw new Error(
        `Marketplace "${name}" has a relative source path (${entry.source.path}) ` +
          ...
          `somacode version. Run 'soma plugin marketplace remove ${name}' and ` +
 ...
 */
export type RefreshAllMarketplacesResult = {
  refreshedCount: number
  skippedSeedManagedCount: number
  skippedSettingsCount: number
  failedCount: number
}

export async function refreshAllMarketplaces(): Promise<RefreshAllMarketplacesResult> {
  const config = await loadKnownMarketplacesConfig()
  const result: RefreshAllMarketplacesResult = {
    refreshedCount: 0,
    skippedSeedManagedCount: 0,
  ...
  for (const [name, entry] of Object.entries(config)) {
    ...
    if (seedDirFor(entry.installLocation)) {
      result.skippedSeedManagedCount++
      logForDebugging(
      ...
      if (resolvedLoc !== cacheDir && !resolvedLoc.startsWith(cacheDir + sep)) {
        throw new Error(
          `Marketplace '${name}' has a corrupted installLocation ` +
            ...
            `Run: soma plugin marketplace remove "${name}" and re-add it.`,
        )
      }
    }
      ...
      try {
        ...
        throw new Error(
          `The marketplace.json file is no longer present in this repository.\n\n` +
            ...
            `You can remove this marketplace with: soma plugin marketplace remove "${name}"`,
        )
      }
