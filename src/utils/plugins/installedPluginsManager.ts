 ...
 *
 * The enabled/disabled state remains in .soma/settings.json for per-repo control.
 *
 ...
 *
 * Legacy cache structure: ~/.soma/plugins/cache/{plugin-name}/
 * Versioned cache structure: ~/.soma/plugins/cache/{marketplace}/{plugin}/{version}/
 *
...
function migrateV1ToV2(v1Data: InstalledPluginsFileV1): InstalledPluginsFileV2 {
  ...
  for (const [pluginId, plugin] of Object.entries(v1Data.plugins)) {
    // V2 format uses versioned cache path: ~/.soma/plugins/cache/{marketplace}/{plugin}/{version}
