 ...
 * user-controlled files (~/.soma/settings.json and ~/.soma.json),
 ...
 *
 * We read from global config (~/.soma.json) and user settings
 * (~/.soma/settings.json). These are user-controlled files that don't
...
function getExtraCertsPathFromConfig(): string | undefined {
  try {
    ...
    // Only read from user-controlled settings (~/.soma/settings.json),
