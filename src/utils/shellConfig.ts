...
import {
  getLegacyLocalCommandPaths,
  getLocalCommandPaths,
} from './localInstaller.js'

export const CLAUDE_ALIAS_REGEX =
  /^\s*alias\s+(?:soma|somacode|soma-code|claude|claude-code)\s*=/
  ...
  const filtered = lines.filter(line => {
    ...
    if (CLAUDE_ALIAS_REGEX.test(line)) {
      ...
      let match = line.match(
        /alias\s+(?:soma|somacode|soma-code|claude|claude-code)\s*=\s*["']([^"']+)["']/,
      )
      if (!match) {
      ...
      if (match && match[1]) {
        ...
        if (installerTargets.has(target)) {
          hadAlias = true
    ...
    for (const line of lines) {
      if (CLAUDE_ALIAS_REGEX.test(line)) {
        ...
        const match = line.match(
          /alias\s+(?:soma|somacode|soma-code|claude|claude-code)=["']?([^"'\s]+)/,
        )
        if (match && match[1]) {
          return match[1]
        }
