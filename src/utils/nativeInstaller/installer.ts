...
import { isNativeInstallerAvailable } from '../distribution.js'
...
function getExecutablePathForCommand(
  commandName: string,
  platform: string = getPlatform(),
): string {
  return join(
    getUserBinDir(),
    platform.startsWith('win32') ? `${commandName}.exe` : commandName,
  )
}

function getExecutablePathsForCommands(
  commandNames: readonly string[],
  platform: string = getPlatform(),
): string[] {
  return commandNames.map(commandName =>
    getExecutablePathForCommand(commandName, platform),
  )
}

function resolveNativeInstallerRoot(
  preferredPath: string,
  legacyPath: string,
): string {
  if (resolve(preferredPath) === resolve(legacyPath)) {
  ...
  try {
    ...
    renameSync(legacyPath, preferredPath)
    return preferredPath
  } catch (error) {
    logError(
      new Error(
        `Failed to migrate native installer path ${legacyPath} -> ${preferredPath}: ${errorMessage(error)}`,
      ),
    )
    return legacyPath
  }
...
function getBaseDirectories() {
  ...
  const dataRoot = resolveNativeInstallerRoot(
    getAppXDGDataDir(),
    getLegacyAppXDGDataDir(),
  )
  const cacheRoot = resolveNativeInstallerRoot(
    getAppXDGCacheDir(),
    getLegacyAppXDGCacheDir(),
  )
  const stateRoot = resolveNativeInstallerRoot(
    getAppXDGStateDir(),
    getLegacyAppXDGStateDir(),
  )
  ...
  return {
    ...
    // User bin
    executable: getExecutablePathForCommand(CLI_COMMAND_NAME, platform),
  }
}
...
async function performVersionUpdate(
  ...
  const platform = getPlatform()
  const executablePaths = getExecutablePathsForCommands(
    CLI_COMMAND_ALIASES,
    platform,
  )
  const legacyExecutablePaths = getExecutablePathsForCommands(
    LEGACY_CLI_COMMAND_ALIASES,
    platform,
  )
  ...
  for (const legacyExecutablePath of legacyExecutablePaths) {
    try {
      if (await isNpmSymlink(legacyExecutablePath)) {
        logForDebugging(
          `Skipping cleanup of ${legacyExecutablePath} - appears to be npm-managed`,
        )
        continue
      }

      await unlink(legacyExecutablePath)
      logForDebugging(`Removed legacy launcher at ${legacyExecutablePath}`)
    } catch (error) {
      if (!isENOENT(error)) {
        logError(
          new Error(
            `Failed to remove legacy launcher ${legacyExecutablePath}: ${error}`,
          ),
        )
      }
    }
  ...
  // Verify the primary executable was actually created/updated
  const primaryExecutablePath = executablePaths[0]
  if (!(await isPossibleClaudeBinary(primaryExecutablePath))) {
    ...
    throw new Error(
      `Failed to create executable at ${primaryExecutablePath}. ` +
        `Source file exists: ${installPathExists}. ` +
        `Check write permissions to ${primaryExecutablePath}.`,
    )
  }
  return needsInstall
}
...
export async function checkInstall(
  ...
  const shouldCheckNative =
    ...
    (isNativeInstallerAvailable() && config.installMethod === 'native')
  ...
  try {
    ...
    messages.push({
      message: `Native install is configured, but the launcher directory is missing: ${localBinDir}. Run \`${CLI_COMMAND_NAME} install\` to repair it.`,
  ...
  if (isWindows) {
    ...
    if (!(await isPossibleClaudeBinary(dirs.executable))) {
      messages.push({
        message: `Native install is configured, but the \`${CLI_COMMAND_NAME}\` launcher is missing or invalid at ${dirs.executable}. Run \`${CLI_COMMAND_NAME} install\` to repair it.`,
        userActionRequired: true,
        type: 'error',
      })
    }
    ...
    try {
      ...
      if (!(await isPossibleClaudeBinary(absoluteTarget))) {
        messages.push({
          message: `${CLI_COMMAND_NAME} symlink points to missing or invalid binary: ${target}`,
          userActionRequired: true,
          type: 'error',
        })
      }
      ...
      if (isENOENT(e)) {
        messages.push({
          message: `Native install is configured, but the \`${CLI_COMMAND_NAME}\` launcher was not found at ${dirs.executable}. Run \`${CLI_COMMAND_NAME} install\` to repair it.`,
          userActionRequired: true,
          type: 'error',
        })
      } else {
        ...
        if (!(await isPossibleClaudeBinary(dirs.executable))) {
          messages.push({
            message: `${dirs.executable} exists but is not a valid ${CLI_COMMAND_NAME} binary`,
            userActionRequired: true,
            type: 'error',
          })
        }
  ...
  if (getPlatform().startsWith('win32')) {
      ...
      try {
        ...
        for (const file of files) {
          if (
            !/^(?:claude|claude-code|soma|somacode|soma-code)\.exe\.old\.\d+$/.test(
              file,
            )
          ) {
            continue
...
export async function removeInstalledSymlink(): Promise<void> {
  ...
  for (const executablePath of executablePaths) {
    try {
      ...
      logForDebugging(`Removed launcher at ${executablePath}`)
    } catch (error) {
      if (isENOENT(error)) {
        continue
      }
      logError(new Error(`Failed to remove launcher ${executablePath}: ${error}`))
    }
  }
}
  ...
  for (const [shellType, configFile] of Object.entries(configMap)) {
    try {
      ...
      if (hadAlias) {
        ...
        messages.push({
          message: `Removed legacy ${LEGACY_CLI_COMMAND_NAME} alias from ${configFile}. Run: unalias ${LEGACY_CLI_COMMAND_NAME}`,
          userActionRequired: true,
          type: 'alias',
        })
        logForDebugging(`Cleaned up legacy ${LEGACY_CLI_COMMAND_NAME} alias from ${shellType} config`)
      }
    } catch (error) {
      logError(error)
      messages.push({
  ...
  try {
    ...
    if (getPlatform().startsWith('win32')) {
      ...
      for (const commandName of CLI_COMMAND_ALIASES) {
        ...
        const binExe = join(globalPrefix, commandName)
      ...
      for (const commandName of CLI_COMMAND_ALIASES) {
        const binSymlink = join(globalPrefix, 'bin', commandName)
...
export async function cleanupNpmInstallations(): Promise<{
  ...
  // Check for local installation at ~/.soma/local
