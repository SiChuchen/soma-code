...
export async function getClaudeDesktopConfigPath(): Promise<string> {
  ...
  if (!SUPPORTED_PLATFORMS.includes(platform)) {
    throw new Error(
      `Unsupported platform: ${platform} - somacode Desktop integration only works on macOS and WSL.`,
    )
  }
  ...
  try {
    ...
    try {
      ...
      // Look for somacode Desktop config in each user directory
  ...
  throw new Error(
    'Could not find somacode Desktop config file in Windows. Make sure somacode Desktop is installed on Windows.',
  )
}
...
> {
  if (!SUPPORTED_PLATFORMS.includes(getPlatform())) {
    throw new Error(
      'Unsupported platform - somacode Desktop integration only works on macOS and WSL.',
    )
  }
  try {
