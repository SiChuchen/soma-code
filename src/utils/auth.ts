...
import type { EndpointApiKeyCredentialSource } from './secureStorage/types.js'
import {
...
import {
  CLAUDE_OFFICIAL_ENDPOINT_ID,
  deleteEndpointCredential,
  getClaudeIdentityCredential,
  getClaudeIdentityCredentialAsync,
...
/**
 * CCR and somacode Desktop spawn the CLI with OAuth and should never fall back
 * to the user's ~/.soma/settings.json API-key config (apiKeyHelper,
...
export function isAnthropicAuthEnabled(): boolean {
  ...
  // ~/.soma settings (apiKeyHelper, settings.env.ANTHROPIC_API_KEY) MUST NOT
  ...
  const is3P = isThirdPartyAPIProvider(getAPIProvider())
  ...
  // On first run, a custom ANTHROPIC_BASE_URL from trusted settings is already
  // applied before onboarding. If a direct key source is also configured, we
  // should treat this as a custom gateway path immediately rather than forcing
  // the official Anthropic preflight/OAuth flow before the key-approval dialog
  // has had a chance to run.
  const hasNonFirstPartyCustomKeySource =
    !isFirstPartyAnthropicBaseUrl() &&
    (apiKeyEnv ||
      process.env.ANTHROPIC_AUTH_TOKEN ||
      apiKeyHelper ||
  ...
  const hasExternalApiKey =
    ...
    apiKeySource === 'CLAUDE_CODE_API_KEY_FILE_DESCRIPTOR' ||
  ...
  const shouldDisableAuth =
    is3P ||
    (hasNonFirstPartyCustomKeySource && !isManagedOAuthContext()) ||
...
export function getAnthropicApiKeyWithSource(
  ...
  if (isEnvTruthy(process.env.CI) || process.env.NODE_ENV === 'test') {
    ...
    if (apiKeyFromFd) {
      return {
        key: apiKeyFromFd,
        source: 'CLAUDE_CODE_API_KEY_FILE_DESCRIPTOR',
      }
    }
 ...
 * from ~/.soma/settings.json or project settings is ignored.
 */
    ...
    refreshProc.on('close', (code, signal) => {
      if (code === 0) {
        ...
        const message = timedOut
          ...
          : chalk.red(
              'Error running awsAuthRefresh (in settings or ~/.soma/.config.json):',
            )
  ...
  try {
    ...
    try {
      ...
      const message = chalk.red(
        'Error getting AWS credentials from awsCredentialExport (in settings or ~/.soma/.config.json):',
      )
    ...
    refreshProc.on('close', (code, signal) => {
      if (code === 0) {
        ...
        const message = timedOut
          ...
          : chalk.red(
              'Error running gcpAuthRefresh (in settings or ~/.soma/.config.json):',
            )
        // biome-ignore lint/suspicious/noConsole:: intentional console output
        console.error(message)
        authStatusManager.endAuthentication(false)
...
export const getApiKeyFromConfigOrMacOSKeychain = memoize(
  (): { key: string; source: ApiKeySource } | null => {
    ...
    const endpointCredential = getEndpointCredential(CLAUDE_OFFICIAL_ENDPOINT_ID)
    if (endpointCredential?.type === 'api_key' && endpointCredential.apiKey) {
      return {
        key: endpointCredential.apiKey,
...
export async function saveApiKey(
  ...
  const endpointId = options.endpointId ?? CLAUDE_OFFICIAL_ENDPOINT_ID

  // Remove the legacy managed-key slot so the new endpoint-scoped credential
  // becomes the only authoritative source moving forward.
  await maybeRemoveApiKeyFromMacOSKeychain()
  const updateStatus = setEndpointApiKeyCredential(endpointId, apiKey, {
    source: options.source,
  })
  if (!updateStatus.success) {
    throw new Error(updateStatus.warning ?? 'Failed to save API key')
  }

  if (getSecureStorage().name === 'keychain') {
  ...
  saveGlobalConfig(current => {
    if (endpointId !== CLAUDE_OFFICIAL_ENDPOINT_ID) {
      return current
    }
...
export async function removeApiKey(
  endpointId: string = CLAUDE_OFFICIAL_ENDPOINT_ID,
): Promise<void> {
  ...
  if (endpointId === CLAUDE_OFFICIAL_ENDPOINT_ID) {
    ...
    // that set keys before we supported endpoint-scoped secure storage.
    saveGlobalConfig(current => ({
      ...current,
      primaryApiKey: undefined,
    }))
...
export function saveOAuthTokensIfNeeded(tokens: OAuthTokens): {
  ...
  try {
    const existingOauth = getClaudeIdentityCredential()

    const updateStatus = setClaudeIdentityCredential({
...
export const getClaudeAIOAuthTokens = memoize((): OAuthTokens | null => {
  ...
  try {
    const oauthData = getClaudeIdentityCredential()
...
export async function getClaudeAIOAuthTokensAsync(): Promise<OAuthTokens | null> {
  ...
  try {
    const oauthData = await getClaudeIdentityCredentialAsync()
    if (!oauthData?.accessToken) {
      return null
    }
    return oauthData
...
export function getStoredOauthAccountInfo(): AccountInfo | undefined {
...
export function getSubscriptionName(): string {
  return getSubscriptionNameForType(getSubscriptionType())
}

function getSubscriptionNameForType(
...
/** Check if using third-party services (Bedrock, Vertex, Foundry, or OpenAI-compatible) */
export function isUsing3PServices(): boolean {
  return isThirdPartyAPIProvider(getAPIProvider())
}
...
export function getAccountInformation() {
  ...
  const storedOauthAccount = getStoredOauthAccountInfo()
  const storedClaudeIdentity = getClaudeIdentityCredential()
  ...
  const hasStoredClaudeIdentity = Boolean(
    storedClaudeIdentity &&
      shouldUseClaudeAIAuth(storedClaudeIdentity.scopes),
  )
  const hasStoredOauthAccount = Boolean(storedOauthAccount)
  const accountInfo: UserAccountInfo = {}
  const { source: apiKeySource } = getAnthropicApiKeyWithSource({
    skipRetrievingKeyFromApiKeyHelper: true,
  })
  const hasManagedApiKey = apiKeySource === '/login managed key'

  if (
    !hasClaudeIdentityToken &&
    !isClaudeAISubscriber() &&
    !hasManagedApiKey &&
    !hasStoredClaudeIdentity &&
    !hasStoredOauthAccount
  ) {
    return undefined
  }
  ...
  if (
    ...
    storedClaudeIdentity?.subscriptionType &&
    hasStoredClaudeIdentity
  ) {
    accountInfo.subscription = getSubscriptionNameForType(
      storedClaudeIdentity.subscriptionType,
    )
  }

  const orgName = storedOauthAccount?.organizationName
  if (orgName) {
    accountInfo.organization = orgName
  }

  const email =
    storedOauthAccount?.emailAddress ??
    (hasStoredClaudeIdentity
      ? storedClaudeIdentity?.profile?.account.email
      : undefined)
  if (email) {
...
export async function validateForceLoginOrg(): Promise<OrgValidationResult> {
  ...
  // in ~/.soma/.config.json is user-writable and cannot be trusted.
  ...
  if (!profile) {
    ...
    return {
      ...
      message:
        ...
        `Try again, or obtain a full-scope token via 'soma auth login'.`,
    }
  }
  ...
  return {
    ...
    message:
      ...
      `Please log in with the correct organization: soma auth login`,
  }
}
