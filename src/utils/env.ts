...
import { getGlobalConfigFileCandidates } from './configPaths.js'
import {
  getAppConfigHomeDir,
  isEnvTruthy,
} from './envUtils.js'
...
export const getGlobalClaudeFile = memoize((): string => {
  for (const candidate of getGlobalConfigFileCandidates()) {
    if (getFsImplementation().existsSync(candidate)) {
      return candidate
    }
  }
  return getGlobalConfigFileCandidates()[0]!
})
