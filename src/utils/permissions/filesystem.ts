...
export function getClaudeSkillScope(
  ...
  const bases = [
    ...
    {
      dir: expandPath(join(getAppConfigHomeDir(), 'skills')),
      prefix: '~/.soma/skills/',
    },
  ]
...
export function isClaudeSettingsPath(filePath: string): boolean {
  ...
  if (
    normalizedPath.endsWith(`${sep}.soma${sep}settings.json`) ||
    normalizedPath.endsWith(`${sep}.soma${sep}settings.local.json`) ||
...
// Always ask when somacode tries to edit its own config files
function isClaudeConfigFilePath(filePath: string): boolean {
  ...
  const commandsDirs = [
    getProjectConfigPath(getOriginalCwd(), 'commands'),
    join(getOriginalCwd(), '.claude', 'commands'),
  ]
  const agentsDirs = [
    getProjectConfigPath(getOriginalCwd(), 'agents'),
    join(getOriginalCwd(), '.claude', 'agents'),
  ]
  const skillsDirs = [
    getProjectConfigPath(getOriginalCwd(), 'skills'),
 ...
 * Path format: ~/.soma/projects/{sanitized-cwd}/...
 */
 ...
 */
export function getAppTempDirName(): string {
  if (getPlatform() === 'windows') {
    return 'claude'
  }
 ...
 * This is a per-user temporary directory used by somacode for all temp files.
 *
...
export const getAppTempDir = memoize(function getAppTempDir(): string {
  const baseTmpDir =
  ...
  return join(resolvedBaseTmpDir, getAppTempDirName()) + sep
})
...
export const getBundledSkillsRoot = memoize(
  function getBundledSkillsRoot(): string {
    ...
    return join(getAppTempDir(), 'bundled-skills', MACRO.VERSION, nonce)
  },
)
...
export function getProjectTempDir(): string {
  return join(getAppTempDir(), sanitizePath(getOriginalCwd())) + sep
}
 ...
 * - MCP CLI state files (managed internally by somacode)
...
export function checkPathSafetyForAutoEdit(
  ...
  for (const pathToCheck of pathsToCheck) {
    if (hasSuspiciousWindowsPathPattern(pathToCheck)) {
      return {
        safe: false,
        message: `somacode requested permissions to write to ${path}, which contains a suspicious Windows path pattern that requires manual approval.`,
  ...
  for (const pathToCheck of pathsToCheck) {
    if (isClaudeConfigFilePath(pathToCheck)) {
      return {
        safe: false,
        message: `somacode requested permissions to write to ${path}, but you haven't granted it yet.`,
  ...
  for (const pathToCheck of pathsToCheck) {
    if (isDangerousFilePathToAutoEdit(pathToCheck)) {
      return {
        safe: false,
        message: `somacode requested permissions to edit ${path} which is a sensitive file.`,
        classifierApprovable: true,
      }
    }
  }
...
export function checkReadPermissionForTool(
  ...
  if (typeof tool.getPath !== 'function') {
    return {
      behavior: 'ask',
      message: `somacode requested permissions to use ${tool.name}, but you haven't granted it yet.`,
    }
  }
  ...
  for (const pathToCheck of pathsToCheck) {
    if (pathToCheck.startsWith('\\\\') || pathToCheck.startsWith('//')) {
      return {
        behavior: 'ask',
        message: `somacode requested permissions to read from ${path}, which appears to be a UNC path that could access network resources.`,
        decisionReason: {
          type: 'other',
  ...
  for (const pathToCheck of pathsToCheck) {
    if (hasSuspiciousWindowsPathPattern(pathToCheck)) {
      return {
        behavior: 'ask',
        message: `somacode requested permissions to read from ${path}, which contains a suspicious Windows path pattern that requires manual approval.`,
        decisionReason: {
          type: 'other',
          reason:
  ...
  for (const pathToCheck of pathsToCheck) {
    ...
    if (askRule) {
      return {
        behavior: 'ask',
        message: `somacode requested permissions to read from ${path}, but you haven't granted it yet.`,
        decisionReason: {
          type: 'rule',
          rule: askRule,
        },
...
export function checkWritePermissionForTool<Input extends AnyObject>(
  ...
  if (claudeFolderAllowRule) {
    ...
    // Accepts both the broad patterns ('/.claude/**', '~/.soma/**') and
    ...
    if (
      ruleContent &&
      allowedPrefixes.some(prefix => ruleContent.startsWith(prefix)) &&
...
export function checkEditableInternalPath(
  ...
  // var value must itself resolve under ~/.soma/jobs/. Symlink guard: every
  ...
  if (feature('TEMPLATES')) {
    ...
    if (jobDir) {
      ...
      // the case where ~/.soma is a symlink (e.g. to /data/claude-config).
  ...
  // ~/.soma/, which is in DANGEROUS_DIRECTORIES. The CLAUDE_COWORK_MEMORY_PATH_OVERRIDE
  ...
  // .claude/ only (not ~/.soma/) since launch.json is per-project.
  if (
    normalizeCaseForComparison(normalizedPath) ===
    normalizeCaseForComparison(getProjectConfigPath(getOriginalCwd(), 'launch.json')) ||
    normalizeCaseForComparison(normalizedPath) ===
      normalizeCaseForComparison(
        join(getLegacyProjectConfigDir(getOriginalCwd()), 'launch.json'),
      )
  ) {
    return {
      behavior: 'allow',
...
export function checkReadableInternalPath(
  ...
  // Tasks directory (~/.soma/tasks/) for swarm task coordination
  ...
  // Teams directory (~/.soma/teams/) for swarm coordination
