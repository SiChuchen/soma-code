...
export function getAllHooks(appState: AppState): IndividualHookConfig[] {
  ...
  if (!restrictedToManagedOnly) {
    ...
    // both resolve to ~/.soma/settings.json)
...
export function hookSourceDescriptionDisplayString(source: HookSource): string {
  switch (source) {
    case 'userSettings':
      return 'User settings (~/.soma/settings.json)'
    case 'projectSettings':
      return 'Project settings (.soma/settings.json)'
    case 'localSettings':
      return 'Local settings (.soma/settings.local.json)'
    case 'pluginHook':
      ...
      // e.g., "Plugin hooks (~/.soma/plugins/repos/source/example-plugin/example-plugin/hooks/hooks.json)"
      return 'Plugin hooks (~/.soma/plugins/*/hooks/hooks.json)'
    case 'sessionHook':
      return 'Session hooks (in-memory, temporary)'
    case 'builtinHook':
      return 'Built-in hooks (registered internally by somacode)'
    default:
