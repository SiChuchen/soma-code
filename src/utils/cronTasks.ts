// Scheduled prompts, stored in <project>/.soma/scheduled_tasks.json.
//
...
export function getCronFilePath(dir?: string): string {
  return getProjectConfigPath(dir ?? getProjectRoot(), 'scheduled_tasks.json')
}

/**
 * Read and parse .soma/scheduled_tasks.json. Returns an empty task list if the file
...
/**
 * Overwrite .soma/scheduled_tasks.json with the given tasks. Creates .soma/ if
...
export async function writeCronTasks(
  ...
  await mkdir(getProjectConfigPath(root), { recursive: true })
 ...
 * written to .soma/scheduled_tasks.json and dies with the process. The
