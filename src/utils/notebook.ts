...
function isLargeOutputs(
  ...
  for (const o of outputs) {
    ...
    if (
      ...
      size += o.image.image_data.length
    }
...
function processOutput(output: NotebookCellOutput): NotebookCellSourceOutput {
  switch (output.output_type) {
    ...
    case 'display_data':
      return {
        output_type: output.output_type,
        text: processOutputText(extractOutputText(output.data)),
...
function cellOutputToToolResult(output: NotebookCellSourceOutput) {
  ...
  if (
    ...
    output.image &&
    (output.image.media_type === 'image/png' ||
      output.image.media_type === 'image/jpeg')
  ) {
    const image = output.image
    const mediaType = image.media_type as 'image/png' | 'image/jpeg'
    outputs.push({
      type: 'image',
      source: {
