// Scheduler lease lock for .soma/scheduled_tasks.json.
//
...
function getLockPath(dir?: string): string {
  return getProjectConfigPath(dir ?? getProjectRoot(), 'scheduled_tasks.lock')
}
...
async function tryCreateExclusive(
  ...
  try {
    ...
    if (code === 'ENOENT') {
      // .soma/ doesn't exist yet — create it and retry once. In steady
