...
/**
 * Builds a deep link URL for somacode Desktop to resume a CLI session.
...
/**
 * Check if somacode Desktop app is installed.
...
/**
 * Detect the installed somacode Desktop version.
...
/**
 * Build and open a deep link to resume the current session in somacode Desktop.
...
export async function openCurrentSessionInDesktop(): Promise<{
  ...
  if (!installed) {
    return {
      success: false,
      error: `somacode Desktop is not installed. See ${PRODUCT_URL}`,
    }
  }
  ...
  if (!opened) {
    return {
      success: false,
      error: 'Failed to open somacode Desktop. Please try opening it manually.',
      deepLinkUrl,
    }
  }
