...
type HighlightJsApi = {
  getLanguage: (languageName: string) => HighlightJsLanguage | undefined
}
...
function getHighlightJsApi(mod: unknown): HighlightJsApi | null {
  if (typeof mod === 'object' && mod !== null) {
    const candidate =
      'default' in mod && typeof mod.default === 'object' && mod.default !== null
        ...
        : mod
    if (
      'getLanguage' in candidate &&
      typeof candidate.getLanguage === 'function'
    ) {
      return candidate as HighlightJsApi
    }
  }
  return null
}
...
async function loadCliHighlight(): Promise<CliHighlight | null> {
  try {
    ...
    loadedGetLanguage = getHighlightJsApi(highlightJs)?.getLanguage
    return {
