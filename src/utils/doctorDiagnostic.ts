...
import { getKnownPackageNames, getPackageName } from './distribution.js'
import { getAppXDGDataDirCandidates } from './xdg.js'
...
const NATIVE_COMMAND_CANDIDATES = [
  ...CLI_COMMAND_ALIASES,
  ...LEGACY_CLI_COMMAND_ALIASES,
]
...
async function getInstallationPath(): Promise<string> {
  ...
  if (isInBundledMode()) {
    ...
    for (const commandName of CLI_COMMAND_ALIASES) {
      try {
        const path = await which(commandName)
        if (path) {
          return path
        }
      } catch {
    ...
    for (const commandName of CLI_COMMAND_ALIASES) {
      try {
        await getFsImplementation().stat(join(homedir(), '.local/bin', commandName))
        return join(homedir(), '.local/bin', commandName)
      } catch {
        // Not found
      }
    }
...
> {
  ...
  const localPath = join(getAppConfigHomeDir(), 'local')
  if (await localInstallationExists()) {
    installations.push({ type: 'npm-local', path: localPath })
  }

  // Check for global npm installation
  const packagesToCheck = getKnownPackageNames()
  ...
  if (npmResult.code === 0 && npmResult.stdout) {
    ...
    for (const commandName of CLI_COMMAND_ALIASES) {
      const candidatePath = isWindows
        ? join(npmPrefix, commandName)
        : join(npmPrefix, 'bin', commandName)
      try {
  ...
  for (const commandName of NATIVE_COMMAND_CANDIDATES) {
    const nativeBinPath = join(homedir(), '.local', 'bin', commandName)
    try {
  ...
  if (config.installMethod === 'native') {
    for (const nativeDataPath of getAppXDGDataDirCandidates()) {
      try {
...
async function detectConfigurationIssues(
  ...
  if (!isEnvTruthy(process.env.DISABLE_INSTALLATION_CHECKS)) {
    if (type === 'npm-local' && config.installMethod !== 'local') {
      warnings.push({
        ...
        fix: `Consider using native installation: ${CLI_COMMAND_NAME} install`,
      })
    }
    ...
    if (type === 'native' && config.installMethod !== 'native') {
      warnings.push({
        ...
        fix: `Run ${CLI_COMMAND_NAME} install to update configuration`,
      })
    }
  }
  ...
  if (type === 'npm-local') {
    // Check if the primary command is already accessible via PATH
    const whichResult = await which(CLI_COMMAND_NAME)
    const somaInPath = !!whichResult

    // Only show warning if soma is NOT in PATH AND no valid alias exists
    if (!somaInPath && !validAlias) {
      if (existingAlias) {
        // Alias exists but points to invalid target
        warnings.push({
          issue: 'Local installation not accessible',
          fix: `Alias exists but points to invalid target: ${existingAlias}. Update alias: alias ${CLI_COMMAND_NAME}="~/.soma/local/${CLI_COMMAND_NAME}"`,
        })
      } else {
        // No alias exists and not in PATH
        warnings.push({
          issue: 'Local installation not accessible',
          fix: `Create alias: alias ${CLI_COMMAND_NAME}="~/.soma/local/${CLI_COMMAND_NAME}"`,
        })
      }
    }
  }
  ...
  if (installationType === 'native') {
    ...
    for (const install of npmInstalls) {
      if (install.type === 'npm-global') {
        ...
        if (getPackageName() !== '@anthropic-ai/claude-code') {
          uninstallCmd += ` && npm -g uninstall ${getPackageName()}`
        }
        warnings.push({
  ...
  if (installationType === 'npm-global') {
    ...
    if (!hasUpdatePermissions && !getAutoUpdaterDisabledReason()) {
      warnings.push({
        ...
        fix: `Do one of: (1) Re-install node without sudo, or (2) Use \`${CLI_COMMAND_NAME} install\` for native installation`,
      })
    }
  }
