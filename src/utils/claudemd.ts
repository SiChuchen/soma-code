 ...
 * 2. User memory (~/.soma/CLAUDE.md) - Private global instructions for all projects
 * 3. Project memory (CLAUDE.md, .soma/CLAUDE.md, and .soma/rules/*.md in project roots) - Instructions checked into the codebase
 ...
 * - CLAUDE.md, .soma/CLAUDE.md, and all .md files in .soma/rules/ are checked in each directory for Project memory
 *
...
import { getProjectConfigPath } from './configPaths.js'
...
/**
 * Processes all .md files in the .soma/rules/ directory and its subdirectories
...
export const getMemoryFiles = memoize(
  async (forceIncludeExternal: boolean = false): Promise<MemoryFileInfo[]> => {
    ...
    if (isSettingSourceEnabled('userSettings')) {
      ...
      // Process User ~/.soma/rules/*.md files
    ...
    // .soma/worktrees/<name>/ from `claude -w`), the upward walk passes
    // through both the worktree root and the main repo root. Both contain
    // checked-in files like CLAUDE.md and .soma/rules/*.md, so the same
    ...
    for (const dir of dirs.reverse()) {
      ...
      if (isSettingSourceEnabled('projectSettings') && !skipProject) {
        ...
        // Try reading .soma/CLAUDE.md (Project)
        const dotClaudePath = getProjectConfigPath(dir, 'CLAUDE.md')
        result.push(
          ...(await processMemoryFile(
            dotClaudePath,
            'Project',
        ...
        // Try reading .soma/rules/*.md files (Project)
        const rulesDir = getProjectConfigPath(dir, 'rules')
        result.push(
          ...(await processMdRules({
            rulesDir,
            type: 'Project',
    ...
    if (isEnvTruthy(process.env.CLAUDE_CODE_ADDITIONAL_DIRECTORIES_CLAUDE_MD)) {
      ...
      for (const dir of additionalDirs) {
        ...
        // Try reading .soma/CLAUDE.md from the additional directory
        const dotClaudePath = getProjectConfigPath(dir, 'CLAUDE.md')
        result.push(
          ...(await processMemoryFile(
            dotClaudePath,
        ...
        // Try reading .soma/rules/*.md files from the additional directory
        const rulesDir = getProjectConfigPath(dir, 'rules')
        result.push(
          ...(await processMdRules({
            rulesDir,
...
export async function getMemoryFilesForNestedDirectory(
  ...
  // Process project memory files (CLAUDE.md and .soma/CLAUDE.md)
  ...
  // Process project unconditional .soma/rules/*.md files, which were not eagerly loaded
  ...
  // Process project conditional .soma/rules/*.md files
  result.push(
    ...(await processConditionedMdRules(
      targetPath,
      rulesDir,
...
export async function processConditionedMdRules(
  ...
  return conditionedRuleMdFiles.filter(file => {
    ...
    // For Project rules: glob patterns are relative to the directory containing .soma
    // For Managed/User rules: glob patterns are relative to the original CWD
    const baseDir =
      type === 'Project'
        ? dirname(dirname(rulesDir)) // Parent of .soma
...
/**
 * Check if a file path is a memory file (CLAUDE.md, CLAUDE.local.md, or .soma/rules/*.md)
 */
export function isMemoryFilePath(filePath: string): boolean {
  ...
  // .md files in .soma/rules/ directories
  if (
    name.endsWith('.md') &&
    (filePath.includes(`${sep}.soma${sep}rules${sep}`) ||
