...
const APP_NAME = 'somacode URL Handler'
const DESKTOP_FILE_NAME = 'claude-code-url-handler.desktop'
const MACOS_APP_NAME = 'somacode URL Handler.app'
...
<dict>
  ...
  <string>${CLI_COMMAND_NAME}</string>
  ...
  <array>
    <dict>
      ...
      <string>somacode Deep Link</string>
...
Name=${APP_NAME}
Comment=Handle ${DEEP_LINK_PROTOCOL}:// deep links for somacode
...
export async function ensureDeepLinkProtocolRegistered(): Promise<void> {
  ...
  // ~/.soma (per-machine, not synced) rather than ~/.soma.json (can sync).
