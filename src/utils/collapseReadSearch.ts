...
type RelevantMemoriesAttachment = AttachmentMessage<{
...
type NestedMemoryAttachment = AttachmentMessage<{
  type: 'nested_memory'
}>

function isRelevantMemoriesAttachment(
  msg: RenderableMessage,
): msg is RelevantMemoriesAttachment {
  return msg.type === 'attachment' && msg.attachment.type === 'relevant_memories'
}

function isNestedMemoryAttachment(
  msg: RenderableMessage,
): msg is NestedMemoryAttachment {
...
export function collapseReadSearchGroups(
  ...
  for (const msg of messages) {
    if (isCollapsibleToolUse(msg, tools)) {
    ...
    } else if (currentGroup.messages.length > 0 && isRelevantMemoriesAttachment(msg)) {
      ...
      const relevantMemories = (msg as RelevantMemoriesAttachment).attachment
        .memories
      currentGroup.relevantMemories ??= []
      currentGroup.relevantMemories.push(...relevantMemories)
