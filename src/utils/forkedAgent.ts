 ...
 * to share the parent's prompt cache. The somacode API cache key is composed of:
  ...
  if (agentId) {
    ...
    lastRecordedUuid =
      initialMessages.length > 0
        ? asUuid(initialMessages[initialMessages.length - 1]!.uuid)
        : null
  }
    ...
    for await (const message of query({
      ...
      if (
        ...
        if (msg.type !== 'progress') {
          lastRecordedUuid = asUuid(msg.uuid)
        }
      }
    }
  } finally {
