...
export async function setupShellCompletion(theme: ThemeName): Promise<string> {
  ...
  try {
    ...
    logError(e)
    return `${EOL}${color('warning', theme)(`Could not write ${shell.name} completion cache`)}${EOL}${chalk.dim(`Run manually: ${CLI_COMMAND_NAME} completion ${shell.shellFlag} > ${shell.cacheFile}`)}${EOL}`
  }
  ...
  const cliBin = process.argv[1] || CLI_COMMAND_NAME
  ...
  if (result.code !== 0) {
    return `${EOL}${color('warning', theme)(`Could not generate ${shell.name} shell completions`)}${EOL}${chalk.dim(`Run manually: ${CLI_COMMAND_NAME} completion ${shell.shellFlag} > ${shell.cacheFile}`)}${EOL}`
  }
  ...
  try {
    ...
    if (
      [...CLI_COMMAND_ALIASES, ...LEGACY_CLI_COMMAND_ALIASES].some(commandName =>
        existing.includes(`${commandName} completion`),
      ) ||
...
/**
 * Regenerate cached shell completion scripts in ~/.soma/.
 * Called after `soma update` so completions stay in sync with the new binary.
 */
