  ...
  for await (const hookResult of executeUserPromptSubmitHooks(
    ...
    if (hookMessage?.type === 'attachment') {
      switch (hookMessage.attachment.type) {
        case 'hook_success': {
          const hookSuccessAttachment = hookMessage.attachment as typeof hookMessage.attachment & {
            content: string;
          };
          if (!hookSuccessAttachment.content) {
          ...
          result.messages.push({
            ...
            attachment: {
              ...
              content: applyTruncation(hookSuccessAttachment.content),
            },
          })
          break
        }
