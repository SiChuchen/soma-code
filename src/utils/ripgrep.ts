...
function getBuiltinRipgrepCommand(): string | null {
  ...
  for (const root of candidateRoots) {
    const command = path.resolve(root, relativeCommand)
...
const getRipgrepConfig = memoize((): RipgrepConfig => {
  ...
  const command = getBuiltinRipgrepCommand()
  if (command) {
  ...
  if (hasSystemRipgrep) {
    logForDebugging(
      'Bundled ripgrep not found in this recovery workspace; falling back to system rg',
    )
    return { mode: 'system', command: 'rg', args: [] }
  }

  const fallbackRoot = path.resolve(__dirname, 'vendor', 'ripgrep')
  const fallbackCommand =
    process.platform === 'win32'
      ? path.resolve(fallbackRoot, `${process.arch}-win32`, 'rg.exe')
      : path.resolve(fallbackRoot, `${process.arch}-${process.platform}`, 'rg')
  return { mode: 'builtin', command: fallbackCommand, args: [] }
})
