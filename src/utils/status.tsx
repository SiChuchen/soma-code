...
import { getAWSRegion, getDefaultVertexRegion } from './envUtils.js';
import { getDisplayPath } from './file.js';
import { formatNumber } from './format.js';
import {
  resolveInferenceClientDescriptor,
  type InferenceClientDescriptor,
} from './inference/clientFactory.js';
import type { InferenceModelRouteSource } from './inference/router.js';
import type { InferenceConfigSource } from './inference/types.js';
...
import { ANTHROPIC_COMPAT_PROFILE_LABELS, getAnthropicCompatConfigFromEnv } from './anthropicCompatConfig.js';
import { getOpenAICompatConfigFromEnv, OPENAI_COMPAT_API_FORMAT_LABELS, OPENAI_COMPAT_PROFILE_LABELS } from './openaiCompatConfig.js';
...
export function getInferenceConfigSourceLabel(source: InferenceConfigSource): string {
  switch (source) {
    case 'inference':
...
export function getInferenceRouteSourceLabel(source: InferenceModelRouteSource): string {
  switch (source) {
    case 'session':
    ...
    case 'base':
      return 'Startup selection';
    case 'inferenceDefault':
      return 'settings.inference default';
    case 'endpointDefault':
...
export function getEndpointAuthLabel(descriptor: Pick<InferenceClientDescriptor, 'endpoint' | 'endpointApiKey' | 'openAICompat'>): string {
  if (descriptor.openAICompat?.disableAuth) {
    return 'Disabled';
  }
  if (descriptor.endpointApiKey) {
    return 'API key (saved)';
  }
  switch (descriptor.endpoint.authMode) {
    case 'claude_oauth':
    ...
    case 'azure_ad':
      return 'Microsoft Entra / Azure AD';
    case 'azure_api_key':
    case 'api_key':
    default:
      return 'API key';
...
export function buildAPIProviderProperties(): Property[] {
  ...
  const properties: Property[] = [];
  const providerLabel = endpoint.kind === 'official_claude' ? 'Official account' : provider === 'bedrock' ? 'AWS Bedrock' : provider === 'vertex' ? 'Google Vertex AI' : provider === 'foundry' ? 'Microsoft Foundry' : provider === 'openaiCompatible' ? 'OpenAI-compatible' : 'Provider-compatible';
  if (endpoint.name !== providerLabel) {
    properties.push({
  ...
  properties.push({
    label: 'Config source',
    value: getInferenceConfigSourceLabel(descriptor.route.inference.source)
  });
  properties.push({
    label: 'Route source',
    value: getInferenceRouteSourceLabel(descriptor.route.source)
  });
  properties.push({
    label: 'Endpoint auth',
    value: getEndpointAuthLabel(descriptor)
  });
  const configuredEndpoints = descriptor.route.inference.endpoints.filter(endpoint => endpoint.enabled);
  if (configuredEndpoints.length > 1) {
    properties.push({
      label: 'Configured endpoints',
      value: configuredEndpoints.map(endpoint => endpoint.id === descriptor.endpoint.id ? `${endpoint.name} (selected)` : endpoint.name)
    });
  }
  if (transportConfig.kind === 'direct' && endpoint.protocol === 'anthropic') {
    const anthropicCompatConfig = getAnthropicCompatConfigFromEnv(process.env);
    if (endpoint.kind !== 'official_claude' && anthropicCompatConfig.profile !== 'manual') {
      properties.push({
        label: 'Compatibility preset',
        value: ANTHROPIC_COMPAT_PROFILE_LABELS[anthropicCompatConfig.profile]
      });
    }
    if (endpoint.kind !== 'official_claude' && transportConfig.baseUrl) {
      properties.push({
        label: 'Compatible base URL',
        value: transportConfig.baseUrl
      });
    }
  } else if (transportConfig.kind === 'bedrock') {
    ...
    properties.push({
      label: 'AWS region',
      value: transportConfig.awsRegion ?? getAWSRegion()
    });
    if (transportConfig.skipAuth) {
      properties.push({
        value: 'AWS auth skipped'
      });
    }
  } else if (transportConfig.kind === 'vertex') {
    if (transportConfig.baseUrl) {
      properties.push({
    ...
    const gcpProject = transportConfig.projectId ?? process.env.ANTHROPIC_VERTEX_PROJECT_ID;
    if (gcpProject) {
      properties.push({
        label: 'GCP project',
        value: gcpProject
    ...
    properties.push({
      ...
      value: transportConfig.region ?? getDefaultVertexRegion()
    });
    if (transportConfig.skipAuth) {
      properties.push({
  ...
  } else if (transportConfig.kind === 'direct' && endpoint.protocol === 'openai') {
    const openAICompatConfig = openAICompat ?? getOpenAICompatConfigFromEnv(process.env);
    const openAIProfileLabel = openAICompatConfig.profile && openAICompatConfig.profile in OPENAI_COMPAT_PROFILE_LABELS ? OPENAI_COMPAT_PROFILE_LABELS[openAICompatConfig.profile] : undefined;
    if (openAICompatConfig.baseUrl) {
      properties.push({
        label: 'OpenAI base URL',
        value: openAICompatConfig.baseUrl
      });
    }
    if (openAICompatConfig.profile && openAICompatConfig.profile !== 'manual' && openAIProfileLabel) {
      properties.push({
        label: 'OpenAI preset',
        value: openAIProfileLabel
      });
    }
    if (openAICompatConfig.model) {
      properties.push({
        label: 'OpenAI model',
        value: openAICompatConfig.model
      });
    }
    if (openAICompatConfig.apiFormat && openAICompatConfig.apiFormat !== 'auto') {
      properties.push({
        label: 'OpenAI format',
        value: OPENAI_COMPAT_API_FORMAT_LABELS[openAICompatConfig.apiFormat]
      });
    }
    if (openAICompatConfig.disableAuth) {
      properties.push({
        value: 'OpenAI auth disabled'
      });
    } else if (openAICompatConfig.apiKeyHeaderName !== 'Authorization' || openAICompatConfig.apiKeyScheme !== 'Bearer') {
      properties.push({
        label: 'OpenAI auth',
        value: openAICompatConfig.apiKeyScheme === '' ? `${openAICompatConfig.apiKeyHeaderName} (raw)` : `${openAICompatConfig.apiKeyHeaderName} (${openAICompatConfig.apiKeyScheme})`
      });
    }
  } else if (transportConfig.kind === 'foundry') {
    if (transportConfig.baseUrl) {
      properties.push({
    ...
    const foundryResource = transportConfig.resource ?? process.env.ANTHROPIC_FOUNDRY_RESOURCE;
    if (foundryResource) {
      properties.push({
