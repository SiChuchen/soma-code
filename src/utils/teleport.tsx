...
You should keep it short and simple, ideally no more than 4 words. The branch should always start with "soma/" and should be all lower case, with words separated by dashes.

Return a JSON object with "title" and "branch" fields.

Example 1: {"title": "Fix login button not working on mobile", "branch": "soma/fix-mobile-login-button"}
Example 2: {"title": "Update README with installation instructions", "branch": "soma/update-readme"}
Example 3: {"title": "Improve performance of data processing script", "branch": "soma/improve-data-processing"}
...
async function generateTitleAndBranch(description: string, signal: AbortSignal): Promise<TitleAndBranch> {
  ...
  const fallbackBranch = 'soma/task';
  try {
...
export async function teleportResumeCodeSession(sessionId: string, onProgress?: TeleportProgressCallback): Promise<TeleportRemoteResponse> {
  ...
  try {
    ...
    if (!accessToken) {
      ...
      throw new Error('somacode web sessions require authentication with a Claude.ai account. API key authentication is not sufficient. Please run /login to authenticate, or check your authentication status with /status.');
    }
      ...
      case 'not_in_repo':
        {
          ...
          throw new TeleportOperationError(`You must run soma --teleport ${sessionId} from a checkout of ${notInRepoDisplay}.`, chalk.red(`You must run soma --teleport ${sessionId} from a checkout of ${chalk.bold(notInRepoDisplay)}.\n`));
        }
      case 'mismatch':
        {
          ...
          throw new TeleportOperationError(`You must run soma --teleport ${sessionId} from a checkout of ${sessionDisplay}.\nThis repo is ${currentDisplay}.`, chalk.red(`You must run soma --teleport ${sessionId} from a checkout of ${chalk.bold(sessionDisplay)}.\nThis repo is ${chalk.bold(currentDisplay)}.\n`));
        }
      case 'error':
  ...
  try {
    ...
    if (!gitSource && bundleSeedGateOn) {
      ...
      if (!bundle.success) {
        ...
        const setup = repoInfo ? '. Please set up GitHub in the web app first' : '';
        let msg: string;
