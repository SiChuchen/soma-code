 ...
 * stopping there makes the parent project's `.soma/` unreachable (#31905).
 *
 ...
 *
 * Worktrees (under `.soma/worktrees/`) stay on the old behavior: their `.git`
 ...
 * collecting all .soma directories along the way.
 *
 * Stopping at git root prevents commands/skills from parent directories outside the repository
 * from leaking into projects. For example, if ~/projects/.soma/commands/ exists, it won't
 ...
 * @returns Array of directory paths containing .soma/subdir, from most specific (cwd) to least specific
 */
export function getProjectDirsUpToHome(
  ...
  while (true) {
    ...
    const claudeSubdir = getProjectConfigPath(current, subdir)
...
export const loadMarkdownFilesForSubdir = memoize(
  async function (
    ...
    // For git worktrees where the worktree does NOT have .soma/<subdir> checked
    ...
    //
    // Only add the main repo's copy when the worktree root's .soma/<subdir>
    // is absent. A standard `git worktree add` checks out the full tree, so the
    // worktree already has identical .soma/<subdir> content — loading the main
    ...
    if (gitRoot && canonicalRoot && canonicalRoot !== gitRoot) {
      const worktreeSubdir = normalizePathForComparison(
        getProjectConfigPath(gitRoot, subdir),
      )
      const worktreeHasSubdir = projectDirs.some(
        dir => normalizePathForComparison(dir) === worktreeSubdir,
      )
      if (!worktreeHasSubdir) {
        const mainClaudeSubdir = getProjectConfigPath(canonicalRoot, subdir)
    ...
    // This prevents the same file from appearing multiple times when ~/.soma is
 ...
 * @param dir Directory (eg. "~/.soma/commands")
 * @returns Array of parsed markdown files with metadata
 */
