...
type TokenUsageMessage = Message | NormalizedMessage
type RoughTokenMessage = Parameters<typeof roughTokenCountEstimationForMessages>[0]

export function getTokenUsage(message: TokenUsageMessage): Usage | undefined {
  if (
 ...
 */
function getAssistantMessageId(message: TokenUsageMessage): string | undefined {
  if (
    message?.type === 'assistant' &&
  ...
  while (i >= 0) {
    ...
    if (message && usage) {
      ...
      return (
        ...
        roughTokenCountEstimationForMessages(messages.slice(i + 1) as RoughTokenMessage)
      )
    }
    i--
  }
  return roughTokenCountEstimationForMessages(messages as RoughTokenMessage)
}
