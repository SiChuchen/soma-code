...
type AgentOrSkillProgress = AgentToolProgress | SkillToolProgress
type ShellProgressData = BashProgress | PowerShellProgress
...
function isAgentOrSkillProgressMessage(
  message: Message,
): message is ProgressMessage<AgentOrSkillProgress> {
  return (
    message.type === 'progress' &&
    hasProgressType(message.data) &&
    (message.data.type === 'agent_progress' ||
      message.data.type === 'skill_progress')
  )
}

function isShellProgressMessage(
  message: Message,
): message is ProgressMessage<ShellProgressData> {
  return (
    message.type === 'progress' &&
    hasProgressType(message.data) &&
...
function normalizeConversationMessages(
  message: Message,
): Array<NormalizedAssistantMessage | NormalizedUserMessage> {
  return normalizeMessages([message] as Message[]).filter(
    (
      normalized,
    ): normalized is NormalizedAssistantMessage | NormalizedUserMessage =>
      normalized.type === 'assistant' || normalized.type === 'user',
  )
}

function toSDKUserMessage(
  message: UserMessage | NormalizedUserMessage,
  parentToolUseID: string | null,
): SDKUserMessage {
  return {
    type: 'user',
    message: message.message as SDKUserMessage['message'],
    ...
    tool_use_result: message.mcpMeta
      ? { content: message.toolUseResult, ...message.mcpMeta }
...
export function* normalizeMessage(message: Message): Generator<SDKMessage> {
  switch (message.type) {
    case 'assistant':
      ...
      return
    case 'progress':
      if (isAgentOrSkillProgressMessage(message)) {
        const progressMessage = message.data.message
        if (progressMessage.type === 'assistant') {
          for (const normalized of normalizeMessages([progressMessage])) {
            // Skip empty messages (e.g., "(no content)") that shouldn't be output to SDK
          ...
          yield toSDKUserMessage(progressMessage, message.parentToolUseID)
        }
      } else if (isShellProgressMessage(message)) {
        // Filter bash progress to send only one per minute
        // Only emit for somacode Remote for now
        if (
      ...
      break
    case 'user':
      for (const normalized of normalizeConversationMessages(message)) {
        if (normalized.type !== 'user') {
          continue
        }
        yield toSDKUserMessage(normalized, null)
      }
      return
    default:
    // yield nothing
