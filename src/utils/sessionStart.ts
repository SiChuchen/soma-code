...
export async function processSessionStartHooks(
  ...
  if (shouldAllowManagedHooksOnly()) {
    ...
    try {
      ...
      if (
        ...
        userGuidance =
          'This appears to be a permissions issue. Check file permissions on ~/.soma/plugins/'
      } else if (
        ...
        userGuidance =
          'This appears to be a configuration issue. Check your plugin settings in .soma/settings.json'
      } else {
        userGuidance =
      ...
      // from .soma/settings.json (loaded via captureHooksConfigSnapshot) will still work
    }
  }
  ...
  for await (const hookResult of executeSessionStartHooks(
    ...
    if (hookResult.message && hookResult.message.type !== 'progress') {
