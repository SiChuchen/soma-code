...
function getErrorDetails(error: unknown): string {
  if (error instanceof Error) {
    ...
    return errnoError.code || error.message;
  }
  return String(error);
}
  ...
  try {
    ...
    const checkEndpoint = async (url: string): Promise<PreflightCheckResult> => {
      try {
        ...
        return {
          success: false,
          error: `Failed to connect to ${hostname}: ${getErrorDetails(error)}`,
    ...
    return {
      success: false,
      error: `Connectivity check error: ${getErrorDetails(error)}`
    };
  }
}
