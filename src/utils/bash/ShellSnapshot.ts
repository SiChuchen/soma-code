...
/**
 * Generates somacode specific snapshot content
...
async function getSnapshotScript(
  ...
  // Generate the user content and somacode content
...
export const createAndSaveSnapshot = async (
  ...
  return new Promise(async resolve => {
    try {
      ...
      if (!configFileExists) {
        logForDebugging(
          `Shell config file not found: ${configFile}, creating snapshot with somacode defaults only`,
        )
      }
      ...
      execFile(
        ...
        async (error, stdout, stderr) => {
          if (error) {
            ...
            logForDebugging(`  - Config home: ${getAppConfigHomeDir()}`)
