...
export function LogSelector(t0: LogSelectorProps) {
  ...
  if ($[94] !== logs || $[95] !== onAgenticSearch || $[96] !== searchQuery) {
    t35 = async () => {
      ...
      if (!searchQuery.trim() || !runAgenticSearch || true) {
        return;
      }
      ...
      ;
      try {
        const searchFn = runAgenticSearch!;
  ...
  const agenticSearchQuery = agenticSearchState.status === "results" ? agenticSearchState.query : undefined;
  const agenticSearchResults = agenticSearchState.status === "results" ? agenticSearchState.results : undefined;
  let t36;
  if ($[98] !== agenticSearchQuery || $[99] !== agenticSearchState.status || $[100] !== searchQuery) {
    t36 = () => {
      if (agenticSearchState.status !== "idle" && agenticSearchState.status !== "searching") {
        if (agenticSearchState.status === "results" && agenticSearchQuery !== searchQuery || agenticSearchState.status === "error") {
          setAgenticSearchState({
            status: "idle"
          });
        }
  ...
  if ($[187] !== agenticSearchResults || $[188] !== agenticSearchState.status) {
    t66 = agenticSearchState.status === "results" && agenticSearchResults && agenticSearchResults.length > 0 && <Box paddingLeft={1} marginBottom={1} flexShrink={0}><Text dimColor={true} italic={true}>somacode found these results:</Text></Box>;
  ...
  if ($[190] !== agenticSearchResults || $[191] !== agenticSearchState.status || $[192] !== filteredLogs) {
    t67 = agenticSearchState.status === "results" && agenticSearchResults && agenticSearchResults.length === 0 && filteredLogs.length === 0 && <Box paddingLeft={1} marginBottom={1} flexShrink={0}><Text dimColor={true} italic={true}>No matching sessions found.</Text></Box>;
  ...
  if ($[197] !== agenticSearchState.status || $[198] !== isAgenticSearchOptionFocused || $[199] !== onAgenticSearch || $[200] !== searchQuery) {
    t69 = Boolean(searchQuery.trim()) && onAgenticSearch && false && agenticSearchState.status !== "searching" && agenticSearchState.status !== "results" && agenticSearchState.status !== "error" && <Box flexShrink={0} flexDirection="column"><Box flexDirection="row" gap={1}><Text color={isAgenticSearchOptionFocused ? "suggestion" : undefined}>{isAgenticSearchOptionFocused ? figures.pointer : " "}</Text><Text color={isAgenticSearchOptionFocused ? "suggestion" : undefined} bold={isAgenticSearchOptionFocused}>Search deeply using somacode →</Text></Box><Box height={1} /></Box>;
