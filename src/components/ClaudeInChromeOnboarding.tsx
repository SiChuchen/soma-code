...
export function ClaudeInChromeOnboarding(t0) {
  ...
  if ($[4] !== isExtensionInstalled) {
    t4 = !isExtensionInstalled && <><Newline /><Newline />Requires a compatible Chrome extension. Open the <Link url={CHROME_EXTENSION_URL}>setup guide</Link>.</>;
  ...
  let t5;
  if ($[6] !== t4) {
    t5 = <Text>somacode in Chrome works with the Chrome extension to let you control your browser directly from somacode. You can navigate websites, fill forms, capture screenshots, record GIFs, and debug with console logs and network requests.{t4}</Text>;
    $[6] = t4;
    $[7] = t5;
  } else {
    t5 = $[7];
  ...
  if ($[8] !== isExtensionInstalled) {
    t6 = isExtensionInstalled && <>{" "}or open the <Link url={CHROME_PERMISSIONS_URL}>permissions page</Link></>;
  ...
  let t7;
  if ($[10] !== t6) {
    t7 = <Text dimColor={true}>Site-level permissions are inherited from the Chrome extension. Manage permissions in the Chrome extension settings to control which sites somacode can browse, click, and type on{t6}.</Text>;
    $[10] = t6;
    $[11] = t7;
  } else {
    t7 = $[11];
  ...
  let t11;
  if ($[17] !== onDone || $[18] !== t10) {
    t11 = <Dialog title="somacode in Chrome (Beta)" onCancel={onDone} color="chromeYellow">{t10}</Dialog>;
    $[17] = onDone;
    $[18] = t10;
    $[19] = t11;
  } else {
