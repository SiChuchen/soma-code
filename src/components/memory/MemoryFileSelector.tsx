...
export function MemoryFileSelector(t0: Props) {
  ...
  const userMemoryPath = join(getAppConfigHomeDir(), "CLAUDE.md");
  ...
  const memoryOptions: OptionWithDescription[] = allMemoryFiles.map(file => {
    ...
    if (file.type === "User" && !file.isNested) {
      description = "Saved in ~/.soma/CLAUDE.md";
    } else {
      if (file.type === "Project" && !file.isNested && file.path === projectMemoryPath) {
  ...
  const folderOptions: OptionWithDescription[] = [];
  const agentDefinitions = useAppState(_temp3);
  if (isAutoMemoryEnabled()) {
    const teamMemModule = teamMemPaths;
    let t1;
    ...
    if (teamMemModule?.isTeamMemoryEnabled()) {
      let t2;
      if ($[1] === Symbol.for("react.memo_cache_sentinel")) {
        t2 = {
          label: "Open team memory folder",
          value: `${OPEN_FOLDER_PREFIX}${teamMemModule.getTeamMemPath()}`,
          description: ""
        };
        $[1] = t2;
      } else {
