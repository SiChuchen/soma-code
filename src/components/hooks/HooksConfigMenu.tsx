...
export function HooksConfigMenu(t0) {
  ...
  if ($[33] !== hooksByEventAndMatcher) {
    ...
    let total = 0;
    for (const [event_0, matchers] of Object.entries(hooksByEventAndMatcher) as Array<[HookEvent, Record<string, IndividualHookConfig[]>]>) {
  ...
  if (hooksDisabled_1) {
    ...
    if ($[53] !== disabledByPolicy) {
      t32 = !disabledByPolicy && <Text dimColor={true}>To re-enable hooks, remove "disableAllHooks" from settings.json or ask somacode.</Text>;
      $[53] = disabledByPolicy;
      $[54] = t32;
    } else {
      t32 = $[54];
