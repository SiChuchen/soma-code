...
export function MCPAgentServerMenu({
  ...
  const agentServerUrl = 'url' in agentServer ? agentServer.url : undefined;
  const agentServerCommand = 'command' in agentServer ? agentServer.command : undefined;
  if (isAuthenticating) {
  ...
  return <Dialog title={`${capitalizedServerName} MCP Server`} subtitle="agent-only" onCancel={onCancel} inputGuide={exitState => exitState.pending ? <Text>Press {exitState.keyName} again to exit</Text> : <Byline>
      ...
      <Box flexDirection="column" gap={0}>
        ...
        {agentServerUrl && <Box>
            <Text bold>URL: </Text>
            <Text dimColor>{agentServerUrl}</Text>
          </Box>}

        {agentServerCommand && <Box>
            <Text bold>Command: </Text>
            <Text dimColor>{agentServerCommand}</Text>
          </Box>}
