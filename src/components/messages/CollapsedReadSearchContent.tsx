...
type ReplToolCallProgress = {
  type: 'repl_tool_call';
  phase: 'start';
  toolInput?: ReplToolCallInput;
  toolName?: string;
};

type ShellProgressLike = {
...
function isReplToolCallProgress(value: unknown): value is ReplToolCallProgress {
  if (!isRecord(value) || value.type !== 'repl_tool_call' || value.phase !== 'start') {
    return false;
  }
  if (value.toolName !== undefined && typeof value.toolName !== 'string') {
    return false;
  }
  return value.toolInput === undefined || isRecord(value.toolInput);
}

function isShellProgress(value: unknown): value is ShellProgressLike {
  return isRecord(value) && (value.type === 'bash_progress' || value.type === 'powershell_progress') && typeof value.elapsedTimeSeconds === 'number' && typeof value.totalLines === 'number';
}

function getReplToolCallHint(progress: ReplToolCallProgress): string | undefined {
  const {
    toolInput
  } = progress;
  ...
  return filePath ?? (pattern ? `"${pattern}"` : undefined) ?? command ?? progress.toolName;
}
...
export function CollapsedReadSearchContent({
  ...
  if (isActiveGroup) {
    for (const id_0 of toolUseIds) {
      ...
      if (isReplToolCallProgress(latest)) {
        incomingHint = getReplToolCallHint(latest);
      }
    }
  }
