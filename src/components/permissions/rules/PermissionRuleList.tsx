...
type RuleTabType = 'allow' | 'ask' | 'deny';
type ValidatedRule = {
...
type GetRulesOptions = (tab: TabType, query?: string) => RulesOptionsResult;
...
type PermissionRulesTabProps = Omit<RulesTabContentProps, 'options' | 'onSelect'> & {
  ...
  handleToolSelect: (value: string, tab: RuleTabType) => void;
};
...
function PermissionRulesTab(t0: PermissionRulesTabProps) {
  ...
  if ($[0] !== t0) {
    ...
    if ($[10] === Symbol.for("react.memo_cache_sentinel")) {
      t8 = {
        allow: "somacode won't ask before using allowed tools.",
        ask: "somacode will always ask for confirmation before using these tools.",
        deny: "somacode will always reject requests to use denied tools."
      };
      $[10] = t8;
    } else {
      t8 = $[10];
...
export function PermissionRuleList(t0) {
  ...
  const getRulesOptions = t6 as GetRulesOptions;
  const exitState = useExitOnCtrlCDWithKeybindings();
  ...
  if ($[30] !== changes || $[31] !== onExit || $[32] !== onRetryDenials) {
    t18 = () => {
      ...
      const denialsFor = (set: Set<number>): AutoModeDenial[] => Array.from(set).map(idx => s_1.denials[idx]).filter(_temp2);
  ...
  if ($[89] === Symbol.for("react.memo_cache_sentinel")) {
    t28 = <Text>somacode can read files in the workspace, and make edits when auto-accept edits is on.</Text>;
    $[89] = t28;
  } else {
    t28 = $[89];
  }
...
function _temp6(opt_0: Option<string>) {
  return opt_0.value;
}
function _temp5(opt: Option<string>) {
