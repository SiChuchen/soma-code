...
export function DesktopHandoff(t0) {
    ...
    t1 = input => {
      ...
      if (state === "prompt-download") {
        if (input === "y" || input === "Y") {
          ...
          onDone(`Opened the somacode repository. Re-run /desktop once a compatible desktop app is installed.`, {
            display: "system"
          });
        } else {
          if (input === "n" || input === "N") {
            onDone(`A compatible desktop app is required for /desktop. Repository: ${PRODUCT_URL}`, {
              display: "system"
            });
          }
        }
    ...
    t2 = () => {
      const performHandoff = async function performHandoff() {
        ...
        if (installStatus.status === "not-installed") {
          setDownloadMessage("somacode Desktop is not installed.");
          setState("prompt-download");
          return;
        }
        if (installStatus.status === "version-too-old") {
          setDownloadMessage(`somacode Desktop needs to be updated (found v${installStatus.version}, need v1.1.2396+).`);
          setState("prompt-download");
          return;
        }
        setState("flushing");
        ...
        if (!result.success) {
          setError(result.error ?? "Failed to open somacode Desktop");
          setState("error");
          return;
        }
        setState("success");
  ...
  if (state === "prompt-download") {
    ...
    if ($[14] === Symbol.for("react.memo_cache_sentinel")) {
      t5 = <Text>Open the repository page now? (y/n)</Text>;
      $[14] = t5;
    } else {
      t5 = $[14];
    }
  ...
  if ($[17] === Symbol.for("react.memo_cache_sentinel")) {
    t4 = {
      checking: "Checking for somacode Desktop\u2026",
      flushing: "Saving session\u2026",
      opening: "Opening somacode Desktop\u2026",
      success: "Opening in somacode Desktop\u2026"
    };
    $[17] = t4;
  } else {
    t4 = $[17];
...
async function _temp2(onDone_0) {
  onDone_0("Session transferred to somacode Desktop", {
    display: "system"
  });
