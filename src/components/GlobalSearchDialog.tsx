...
export function GlobalSearchDialog(t0: Props) {
  ...
  if ($[24] !== preview || $[25] !== previewWidth || $[26] !== query) {
    t14 = m_8 => {
      const activePreview = preview?.file === m_8.file && preview?.line === m_8.line ? preview : null;
      return activePreview ? <><Text dimColor={true}>{truncatePathMiddle(m_8.file, previewWidth)}:{m_8.line}</Text>{activePreview.content.split("\n").map((line_0, i) => <Text key={i}>{highlightMatch(truncateToWidth(line_0, previewWidth), query)}</Text>)}</> : <LoadingState message={"Loading\u2026"} dimColor={true} />;
    };
    $[24] = preview;
    $[25] = previewWidth;
    $[26] = query;
