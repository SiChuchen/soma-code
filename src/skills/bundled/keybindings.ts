...
const SECTION_INTRO = [
  '# Keybindings Skill',
  '',
  'Create or modify `~/.soma/keybindings.json` to customize keyboard shortcuts.',
  '',
  '## CRITICAL: Read Before Write',
  '',
  '**Always read `~/.soma/keybindings.json` first** (it may not exist yet). Merge changes with existing bindings — never replace the entire file.',
  '',
...
const SECTION_DOCTOR = [
  ...
  '',
  'The `/doctor` command includes a "Keybinding Configuration Issues" section that validates `~/.soma/keybindings.json`.',
  '',
  ...
  'Location: ~/.soma/keybindings.json',
...
export function registerKeybindingsSkill(): void {
  registerBundledSkill({
    ...
    description:
      'Use when the user wants to customize keyboard shortcuts, rebind keys, add chord bindings, or modify ~/.soma/keybindings.json. Examples: "rebind ctrl+s", "add a chord shortcut", "change the submit key", "customize keybindings".',
    allowedTools: ['Read'],
    userInvocable: false,
