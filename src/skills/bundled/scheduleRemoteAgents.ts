...
function formatConnectorsInfo(connectors: ConnectorInfo[]): string {
  if (connectors.length === 0) {
    return 'No connected MCP connectors found. The user may need to connect servers in the web app settings.'
  }
...
You are helping the user schedule, update, list, or run **remote** somacode agents. These are NOT local cron jobs — each trigger spawns a fully isolated remote session (CCR) in the remote cloud infrastructure on a cron schedule. The agent runs in a sandboxed environment with its own git checkout, tools, and optional MCP connections.
...
You CANNOT delete triggers. If the user asks to delete, direct them to the Scheduled page in the web app.
...
**Important:** Infer what services the agent needs from the user's description. For example, if they say "check Datadog and Slack me errors," the agent needs both Datadog and Slack connectors. Cross-reference against the list above and warn if any required service isn't connected. If a needed connector is missing, direct the user to the connectors settings page in the web app to connect it first.
...
5. **Validate connections** — Infer what services the agent will need from the user's description. For example, if they say "check Datadog and Slack me errors," the agent needs both Datadog and Slack MCP connectors. Cross-reference with the connectors list above. If any are missing, warn the user and direct them to the connectors settings page in the web app to connect first.${gitRepoUrl ? ` The default git repo is already set to \`${gitRepoUrl}\`. Ask the user if this is the right repo or if they need a different one.` : ' Ask which git repos the remote agent needs cloned into its environment.'}
6. **Review and confirm** — Show the full configuration before creating. Let them adjust.
7. **Create it** \u2014 Call \`${REMOTE_TRIGGER_TOOL_NAME}\` with \`action: "create"\` and show the result. The response includes the trigger ID. Always tell the user they can review it from the Scheduled page in the web app.
...
- These are REMOTE agents — they run in the remote cloud, not on the user's machine. They cannot access local files, local services, or local environment variables.
...
- To delete a trigger, direct users to the Scheduled page in the web app
...
export function registerScheduleRemoteAgentsSkill(): void {
  registerBundledSkill({
    ...
    whenToUse:
      'When the user wants to schedule a recurring remote agent, set up automated tasks, create a cron job for somacode, or manage their scheduled agents/triggers.',
    userInvocable: true,
    isEnabled: () =>
    ...
    async getPromptForCommand(args: string, context: ToolUseContext) {
      if (!getClaudeAIOAuthTokens()?.accessToken) {
        return [
          {
            type: 'text',
            text: 'You need to authenticate with your web account first. API accounts are not supported. Run /login, then try /schedule again.',
          },
        ]
      }
      ...
      if (environments.length === 0) {
        try {
          ...
          return [
            {
              type: 'text',
              text: 'No remote environments found, and we could not create one automatically. Set one up in the web app, then run /schedule again.',
            },
          ]
        }
      }
      ...
      if (repo === null) {
        ...
        if (!hasAccess) {
          ...
          const msg = webSetupEnabled
            ? `GitHub not connected for ${repo.owner}/${repo.name} \u2014 run /web-setup to sync your GitHub credentials, or install the Claude GitHub App from the web app onboarding flow.`
            : `Claude GitHub App not installed on ${repo.owner}/${repo.name} \u2014 install it from the web app onboarding flow if your trigger needs this repo.`
          setupNotes.push(msg)
        }
      }
