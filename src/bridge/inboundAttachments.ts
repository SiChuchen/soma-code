 ...
 * (oauth-authed, same store), write to ~/.soma/uploads/{sessionId}/, and
...
import { asContentBlockParams } from '../utils/contentBlockCompat.js'
...
export function prependPathRefs(
  content: string | Array<ContentBlockParam> | Array<BetaContentBlockParam>,
  prefix: string,
): string | Array<ContentBlockParam> {
  const normalizedContent =
    typeof content === 'string' ? content : asContentBlockParams(content)
  if (!prefix) return normalizedContent
  if (typeof normalizedContent === 'string') return prefix + normalizedContent
  const i = normalizedContent.findLastIndex(b => b.type === 'text')
  if (i !== -1) {
    const b = normalizedContent[i]!
    if (b.type === 'text') {
      return [
        ...normalizedContent.slice(0, i),
        { ...b, text: prefix + b.text },
        ...normalizedContent.slice(i + 1),
      ]
    }
  }
  // No text block — append one at the end so it's last.
  return [...normalizedContent, { type: 'text', text: prefix.trimEnd() }]
}
...
export async function resolveAndPrepend(
  ...
  if (attachments.length === 0) {
    return typeof content === 'string' ? content : asContentBlockParams(content)
  }
