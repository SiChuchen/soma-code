...
import {
  asContentBlockParams,
  type CompatibleContentBlockParam,
} from '../utils/contentBlockCompat.js'
...
export function normalizeImageBlocks(
  blocks: Array<CompatibleContentBlockParam>,
): Array<ContentBlockParam> {
  if (!blocks.some(isMalformedBase64Image)) {
    return asContentBlockParams(blocks)
  }

  return blocks.map(block => {
    if (!isMalformedBase64Image(block)) return block as ContentBlockParam
