...
function getDefaultTeammateModel(leaderModel: string | null): string {
  ...
  // Default behavior: follow the leader's current model unless a specific
  // teammate model was configured globally or passed explicitly on the tool call.
  return leaderModel ?? getHardcodedTeammateModelFallback()
}
...
async function handleSpawnSplitPane(
  ...
  // Resolve model: 'inherit' → leader's model; undefined → leader's model
  ...
  // Build the command to spawn somacode with teammate identity
...
/**
 * Handle spawn operation - creates a new somacode instance.
