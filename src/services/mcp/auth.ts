 ...
 *
 * Per RFC 7009, public clients (like somacode) should authenticate by including
...
async function performMCPXaaAuth(
  ...
  if (!idp) {
    throw new Error(
      "XAA: no IdP connection configured. Run 'soma mcp xaa setup --issuer <url> --client-id <id> --client-secret' to configure.",
    )
  }
      ...
      server = createServer((req, res) => {
        ...
        if (parsedUrl.pathname === '/callback') {
          ...
          if (code) {
            res.writeHead(200, { 'Content-Type': 'text/html' })
            res.end(
              `<h1>Authentication Successful</h1><p>You can close this window. Return to somacode.</p>`,
            )
            cleanup()
            resolveOnce(code)
          }
...
export class ClaudeAuthProvider implements OAuthClientProvider {
  ...
  get clientMetadata(): OAuthClientMetadata {
    const metadata: OAuthClientMetadata = {
      client_name: `somacode (${this.serverName})`,
