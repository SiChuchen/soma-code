...
/**
 * Is this plugin enabled (value === true) in .soma/settings.json?
 *
...
export async function uninstallPluginOp(
  ...
  if (!scopeInstallation) {
    ...
    if (actualScope !== scope && installations && installations.length > 0) {
      // Project scope is special: .soma/settings.json is shared with the team.
      // Point users at the local-override escape hatch instead of --scope project.
      if (actualScope === 'project') {
        return {
          success: false,
          message: `Plugin "${plugin}" is enabled at project scope (.soma/settings.json, shared with your team). To disable just for you: soma plugin disable ${plugin} --scope local`,
        }
      }
      return {
        success: false,
...
export async function setPluginEnabledOp(
  ...
  // without touching the shared .soma/settings.json).
