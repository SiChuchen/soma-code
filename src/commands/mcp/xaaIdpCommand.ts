...
export function registerMcpXaaIdpCommand(mcp: Command): void {
  ...
  xaaIdp
    ...
    .requiredOption('--client-id <id>', "somacode's client_id at the IdP")
    .option(
      '--client-secret',
  ...
  xaaIdp
    ...
    .action(async options => {
      ...
      if (!idp) {
        return cliError(
          `Error: no XAA IdP connection. Run '${CLI_COMMAND_NAME} mcp xaa setup' first.`,
        )
      }
  ...
  xaaIdp
    ...
    .action(() => {
      ...
      process.stdout.write(
        `Logged in:     ${hasIdToken ? 'yes (id_token cached)' : `no — run '${CLI_COMMAND_NAME} mcp xaa login'`}\n`,
      )
      cliOk()
    })
