...
async function createWorkflowFile(
  ...
  branchName: string,
  workflowPath: string,
  legacyWorkflowPath: string | null,
  ...
  const workflowPathCandidates = legacyWorkflowPath
    ? [workflowPath, legacyWorkflowPath]
    : [workflowPath]

  let fileSha: string | null = null
  let targetWorkflowPath = workflowPath
  for (const candidatePath of workflowPathCandidates) {
    const checkFileResult = await execFileNoThrow('gh', [
      'api',
      `repos/${repoName}/contents/${candidatePath}`,
      '--jq',
      '.sha',
    ])
    if (checkFileResult.code === 0) {
      fileSha = checkFileResult.stdout.trim()
      targetWorkflowPath = candidatePath
      break
    }
  }
  ...
  const apiParams = [
    ...
    '--method',
    'PUT',
    `repos/${repoName}/contents/${targetWorkflowPath}`,
    '-f',
  ...
  if (createFileResult.code !== 0) {
    if (
      ...
      throw new Error(
        `Failed to create workflow file ${targetWorkflowPath}: An existing workflow file already exists in this repository. Please remove it first or update it manually.`,
      )
    }
    ...
    const helpText =
      ...
      '· Manual setup → Use the somacode workflow template if you prefer to finish this by hand'

    throw new Error(
      `Failed to create workflow file ${targetWorkflowPath}: ${createFileResult.stderr}${helpText}`,
    )
  }
}
...
export async function setupGitHubActions(
  ...
  try {
    ...
    if (!skipWorkflow) {
      updateProgress()
      // Create new branch
      branchName = `add-somacode-github-actions-${Date.now()}`
      ...
      if (selectedWorkflows.includes('claude')) {
        workflows.push({
          ...
          message: 'somacode PR Assistant workflow',
        })
      }
      ...
      if (selectedWorkflows.includes('claude-review')) {
        workflows.push({
          ...
          message: 'somacode Review workflow',
        })
      }
      ...
      for (const workflow of workflows) {
        await createWorkflowFile(
          ...
          branchName,
          workflow.path,
          workflow.path === PRIMARY_WORKFLOW_PATH
            ? LEGACY_PRIMARY_WORKFLOW_PATH
