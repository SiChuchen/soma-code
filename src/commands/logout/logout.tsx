...
import { clearAllInferenceCredentials, deleteClaudeIdentityCredential } from '../../utils/inference/credentials.js';
...
export async function performLogout({
  ...
  endpointId?: string;
  scope?: 'claude' | 'endpoint' | 'allInference';
}): Promise<void> {
  ...
  if (scope === 'endpoint') {
    if (!endpointId) {
      throw new Error('endpointId is required when scope is "endpoint"');
    }
    await removeApiKey(endpointId);
  } else if (scope === 'allInference') {
    await removeApiKey();
  ...
  saveGlobalConfig(current => {
    ...
    if (scope !== 'endpoint' && clearOnboarding) {
...
export async function call(): Promise<React.ReactNode> {
  ...
  const message = <Text>Successfully logged out from your Claude account.</Text>;
  setTimeout(() => {
