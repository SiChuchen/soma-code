...
export function ManageMarketplaces({
  ...
  const applyChanges = async (states?: MarketplaceState[]) => {
    ...
    try {
      ...
      const skippedSettingsMarketplaces: string[] = [];
      ...
      for (const state of statesToProcess) {
        ...
        if (state.pendingUpdate) {
          if (state.sourceType === 'settings') {
            skippedSettingsCount++;
            skippedSettingsMarketplaces.push(state.name);
            continue;
          }
      ...
      if (updatedCount > 0) {
        ...
        const skippedSettingsPart = skippedSettingsCount > 0 ? ` (${skippedSettingsCount} settings-sourced ${plural(skippedSettingsCount, 'marketplace')} skipped)` : '';
        actions.push(`Updated ${updatedCount} ${plural(updatedCount, 'marketplace')}${pluginPart}${skippedSettingsPart}`);
      }
      if (removedCount > 0) {
      ...
      if (actions.length > 0) {
        successMsg = `${figures.tick} ${actions.join(', ')}`;
      } else if (skippedSettingsCount > 0) {
        successMsg = skippedSettingsCount === 1 ? `Marketplace '${skippedSettingsMarketplaces[0]}' is settings-sourced and has no upstream to refresh` : `No marketplaces were updated (${skippedSettingsCount} settings-sourced ${plural(skippedSettingsCount, 'marketplace')} skipped)`;
      }
      if (successMsg) {
  ...
  if (internalView === 'details' && selectedMarketplace) {
    ...
    return <Box flexDirection="column">
        ...
        {!isUpdating && !shouldSkipPluginAutoupdate() && selectedMarketplace.autoUpdate && <Box marginTop={1}>
              <Text dimColor>
                Auto-update enabled. somacode will automatically update this
