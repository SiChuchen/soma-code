...
import { InferenceModelPicker } from '../../components/InferenceModelPicker.js';
...
import { deriveInferenceModelPickerState, getInferenceModelDisplayLabel, normalizeInferenceModelSelection } from '../../utils/inference/modelPicker.js';
import { resolveInferenceConfig } from '../../utils/inference/resolve.js';
import { MODEL_ALIASES } from '../../utils/model/aliases.js';
import { checkOpus1mAccess, checkSonnet1mAccess } from '../../utils/model/check1mAccess.js';
import { getDefaultMainLoopModelSetting, isOpus1mMergeEnabled, renderDefaultModelSetting } from '../../utils/model/model.js';
import { isModelAllowed } from '../../utils/model/modelAllowlist.js';
import { getApiConfigLegacyEnv } from '../../utils/settings/apiConfig.js';
import type { SettingsJson } from '../../utils/settings/types.js';
import { validateModel } from '../../utils/model/validateModel.js';
type ModelCommandSettings = {
  readonly api?: unknown;
  readonly env?: Record<string, string | undefined>;
  readonly inference?: unknown;
  readonly model?: string | null;
} | null | undefined;
function getInferenceModelPickerStateFromSettings(settings: ModelCommandSettings): ReturnType<typeof deriveInferenceModelPickerState> {
  return deriveInferenceModelPickerState({
    inference: settings?.inference as SettingsJson['inference'] | undefined,
    legacyEnv: getApiConfigLegacyEnv(settings?.env ?? {}),
...
function ModelPickerWrapper(t0) {
  ...
  const inferenceRemoteModelResolver = React.useMemo(() => {
    ...
    const resolved = resolveInferenceConfig({
      inference: inferenceModelPickerState.editableInference
    });
    const remoteModelsById = new Map(inferenceModelOptions.map(option => [option.value, option.remoteModel]));
    return (model: string | null): string | undefined => {
      if (model === null) {
        return resolved.selectedModel.remoteModel;
      }
  ...
  function handleSelect(model: string | null, effort: EffortLevel | undefined): void {
    ...
    let message = `Set model to ${chalk.bold(renderModelLabel(model, inferenceModelPickerState))}`;
    if (effort !== undefined) {
...
function SetModelAndClose({
  ...
  React.useEffect(() => {
    async function handleModelChange(): Promise<void> {
      const configuredInferenceModel = inferenceModelPickerState.shouldUseInferenceModelPicker ? normalizeInferenceModelSelection(inferenceModelPickerState.editableInference, model) : undefined;
      if (configuredInferenceModel) {
        setModel(configuredInferenceModel);
        return;
      }
    ...
    function setModel(modelValue: string | null): void {
      ...
      let message = `Set model to ${chalk.bold(renderModelLabel(modelValue, inferenceModelPickerState))}`;
      let wasFastModeToggledOn: boolean | undefined = undefined;
      if (isFastModeEnabled()) {
        clearFastModeCooldown();
  ...
  }, [inferenceModelPickerState, isFastMode, model, onDone, setAppState]);
  return null;
}
...
function ShowModelAndClose(t0) {
  ...
  if (mainLoopModelForSession) {
    onDone(`Current model: ${chalk.bold(renderModelLabel(mainLoopModelForSession, inferenceModelPickerState))} (session override)\nBase model: ${displayModel}${effortInfo}`);
  } else {
...
function ClearSessionModelOverrideAndClose(t0) {
  const {
    onDone
  } = t0;
  const mainLoopModel = useAppState(_temp10);
  const mainLoopModelForSession = useAppState(_temp11);
  const settings = useAppState(s => s.settings);
  const setAppState = useSetAppState();
  const inferenceModelPickerState = React.useMemo(() => getInferenceModelPickerStateFromSettings(settings), [settings]);
  React.useEffect(() => {
    if (!mainLoopModelForSession) {
      onDone('No session model override is active.', {
        display: 'system'
      });
      return;
    }
    ...
    onDone(`Cleared session model override. Base model: ${chalk.bold(renderModelLabel(mainLoopModel, inferenceModelPickerState))}`, {
      display: 'system'
    });
  }, [inferenceModelPickerState, mainLoopModel, mainLoopModelForSession, onDone, setAppState]);
  return null;
}
function _temp11(s_1) {
  return s_1.mainLoopModelForSession;
}
function _temp10(s) {
  return s.mainLoopModel;
}
function parseModelCommandArgs(args: string): {
  ...
  if (args.startsWith('default ')) {
    return {
      kind: 'set',
      value: args.slice('default '.length).trim()
    };
  }
  return {
    kind: 'set',
...
export const call: LocalJSXCommandCall = async (onDone, _context, args) => {
  ...
  if (COMMON_HELP_ARGS.includes(args)) {
    onDone('Run /model to open the model selection menu, /model [modelName] to set a session model, /model default [modelName] to manage the persisted default, or /model clear to drop a session override.', {
      display: 'system'
    });
    return;
  }
  if (args) {
    ...
    if (parsed.kind === 'clear') {
      return <ClearSessionModelOverrideAndClose onDone={onDone} />;
    }
    return <SetModelAndClose args={parsed.value ?? args} onDone={onDone} />;
  }
  return <ModelPickerWrapper onDone={onDone} />;
};
function renderModelLabel(model: string | null, inferenceModelPickerState?: ReturnType<typeof deriveInferenceModelPickerState>): string {
  const inferenceLabel = inferenceModelPickerState?.shouldUseInferenceModelPicker ? getInferenceModelDisplayLabel(inferenceModelPickerState.editableInference, model) ?? getInferenceModelDisplayLabel(inferenceModelPickerState.editableInference, null) : undefined;
  if (inferenceLabel) {
    return model === null ? `${inferenceLabel} (default)` : inferenceLabel;
  }
