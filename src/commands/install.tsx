...
import { getGlobalInstallCommand, isNativeInstallerAvailable } from '../utils/distribution.js';
...
function getInstallationPath(): string {
  ...
  if (isWindows) {
    ...
    const windowsPath = join(homeDir, '.local', 'bin', `${CLI_COMMAND_NAME}.exe`);
    // Replace forward slashes with backslashes for Windows display
    return windowsPath.replace(/\//g, '\\');
  }
  return `~/.local/bin/${CLI_COMMAND_NAME}`;
}
...
function Install({
    ...
    async function run() {
      try {
        ...
        if (!isNativeInstallerAvailable()) {
          throw new Error(
            `Native installer artifacts are not published for this distribution yet. Use ${getGlobalInstallCommand()} instead.`,
          );
        }
        ...
        if (result.lockFailed) {
          throw new Error('Could not install - another process is currently installing somacode. Please try again in a moment.');
        }
  ...
  useEffect(() => {
    if (state.type === 'success') {
      ...
      setTimeout(onDone, 2000, 'somacode installation completed successfully', {
        display: 'system' as const
      });
    } else if (state.type === 'error') {
      // Give error message time to render before exiting
      setTimeout(onDone, 3000, 'somacode installation failed', {
        display: 'system' as const
      });
    }
  }, [state, onDone]);
  return <Box flexDirection="column" marginTop={1}>
      ...
      {state.type === 'installing' && <Text color="claude">
          Installing somacode native build {getDisplayVersion(state.version)}...
        </Text>}
      ...
      {state.type === 'success' && <Box flexDirection="column" gap={1}>
          <Box>
            ...
            <Text color="success" bold>
              somacode successfully installed!
            </Text>
          </Box>
          <Box marginLeft={2} flexDirection="column" gap={1}>
            {state.version !== 'current' && <Box>
                <Text dimColor>Version: </Text>
                <Text color="claude">{getDisplayVersion(state.version)}</Text>
              </Box>}
            <Box>
...
export const install = {
  ...
  name: 'install',
  description: 'Install somacode native build',
