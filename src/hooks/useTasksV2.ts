...
export function useTasksV2WithCollapseEffect(): Task[] | undefined {
  ...
  const hasVisibleIncompleteTasks =
    tasks?.some(task => task.status !== 'completed') ?? false
  useEffect(() => {
    if (!hidden && hasVisibleIncompleteTasks) return
    setAppState(prev => {
      if (prev.expandedView !== 'tasks') return prev
      return { ...prev, expandedView: 'none' as const }
    })
  }, [hidden, hasVisibleIncompleteTasks, setAppState])
