...
const BUILD_NODE_ENV = 'production' as 'production' | 'development' | 'test';
...
export async function showSetupScreens(root: Root, permissionMode: PermissionMode, allowDangerouslySkipPermissions: boolean, commands?: Command[], claudeInChrome?: boolean, devChannels?: ChannelEntry[]): Promise<boolean> {
  if (BUILD_NODE_ENV === 'test' || isEnvTruthy(false) || process.env.IS_DEMO // Skip onboarding in demo mode
  ) {
    return false;
  }
  ...
  // Show Chrome onboarding for first-time somacode in Chrome users
